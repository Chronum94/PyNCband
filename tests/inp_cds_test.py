...
def main():
    ...
    for i, shell_width in enumerate(shell_widths):
        ...
        whole_integral_energy = csnc.bandgap + np.sum(energies) + col_energy_whole[0] + pol_energy_whole[0]
        ...
        print(
            "{:0.2f}({:0.2f})".format(
                whole_integral_energy, abs(experimental_bandgaps[i] - whole_integral_energy)
            ),
            end="\t",
        )
        print(
            "{:0.2f} ({:0.2f})".format(
                sectioned_integral_energy, abs(experimental_bandgaps[i] - sectioned_integral_energy)
            ),
            end="\t",
        )
        print()
