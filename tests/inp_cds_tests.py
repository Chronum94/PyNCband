...
def main():
    ...
    core_widths = np.array([1.23, 1.23, 1.23, 1.23, 1.225, 1.225])
    # shell_widths_optimized = np.array([0.53, 1.05, 1.35, 1.60, 2.26, 3.39])
    ...
    for i, shell_width in enumerate(shell_widths):
        csnc = CoreShellParticle(InP, CdS, core_widths[i], shell_width, 1.5)
        print(core_widths[i], "\t", shell_width, end="\t")
        # print("Is CSNC type two? h/e?", csnc.type_two, csnc.h_e)
        energies = np.array(csnc.calculate_s1_energies())
        # print(energies)
        plots = False
        fig, ax = plt.subplots(1, 2)
        col_energy_sectioned, col_plotaxis = csnc.coulomb_screening_energy(plot_integrand=ax[0], cmap='coolwarm_r')
        pol_energy_sectioned, pol_plotaxis = csnc.interface_polarization_energy(plot_integrand=ax[1], cmap='coolwarm_r')
        # fig.colorbar(col_plotaxis)
        # fig.colorbar(pol_plotaxis)
        plt.show()
        # whole_integral_energy = (
        #     csnc.bandgap + np.sum(energies) + col_energy_whole[0] + pol_energy_whole[0]
        # )
        sectioned_integral_energy = csnc.bandgap + np.sum(energies) + col_energy_sectioned[0] + pol_energy_sectioned[0]
