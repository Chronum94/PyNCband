...
from pyncband import Material, CoreShellParticle2 as CoreShellParticle
...
def test_overlap_integrals_are_consistent():
    ...
    csnc1 = CoreShellParticle(a, b, 1.0, 1.0)  # Type 2 CSNC. h/e structure.
    assert np.isclose(
        csnc1.analytical_overlap_integral(), csnc1.numerical_overlap_integral()
    )
...
def test_asymptotic_core_localization():
    ...
    assert np.isclose(
        csnc1.localization_hole_core(50000), csnc1.localization_hole_core(asymp=True)
    )
    assert np.isclose(
        csnc2.localization_electron_core(50000),
        csnc2.localization_electron_core(asymp=True),
    )
...
def test_asymptotic_shell_localization():
    ...
    assert np.isclose(
        csnc2.localization_hole_shell(1e6), csnc2.localization_hole_shell(asymp=True)
    )
