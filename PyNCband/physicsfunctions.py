...
__all__ = ['core_wavefunction', 'shell_wavefunction', 'wavevector_from_energy',
...
def electron_eigenvalue_residual(energy, particle):
    if particle.eh:
        ...
        q_e = wavevector_from_energy(energy, particle.smat.m_e, potential_offset=particle.ue)
        core_x = k_e * particle.core_width
        with np.errstate(divide='ignore', invalid='ignore'):
            return  (1 - core_x / np.tan(core_x)) * particle.smat.m_e / particle.cmat.m_e - 1 - q_e * particle.core_width / np.tan(q_e * particle.shell_width)